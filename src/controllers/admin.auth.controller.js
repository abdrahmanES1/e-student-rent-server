...
const register = asyncHandler(async (req, res, next) => {
    const { username, email, password, adresse, phone } = req.body;
    if (!(username && email && password && adresse && phone)) { return next(new ErrorResponse("All Fields is Required", 403)); }

    if (await UserModel.findOne({ email })) {
        return next(new ErrorResponse("Email Already Exist", 403));
    }

    const user = await UserModel.create({ username, email, password, adresse, phone })
